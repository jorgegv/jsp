	...
	public jsp_sprite_smc_4
	public jsp_sprite_smc_5
...
; some operations are patched by SMC code in this routine, they are labeled
; as such
;
...
jsp_draw_sprite_j:
	...
	;     pix_ptr = pix_ptr_left = sp->pixels - ( ypos % 8 );	// LOAD1
	;     pix_ptr = pix_ptr_left = sp->pixels - ( ypos % 8 ) * 2;	// MASK2
	...
	ld h,0			; HL = ( ypos % 8 )
jsp_sprite_smc_1:
	nop			; SMC: patched to NOP (LOAD1) or ADD HL,HL (MASK2)
	ld e,(ix+5)
	ld d,(ix+6)		; DE = sp->pixels
	ex de,hl
	sbc hl,de		; HL = sp->pixels - ( ypos % 8 ) // or * 2 for MASK2
	...
	;     bg_ptr = &sp->pdbuf[ 0 ];
	;     for ( i = 0; i < sp->rows + 1; i++ ) {
	;         sp1_draw_load1lb( bg_ptr, pix_ptr );	// or sp1_draw_mask2
	;         bg_ptr += ( sp->cols + 1 ) * 8;
	;         pix_ptr += 8;	// or 16 for MASK2
	;     }
...
jsp_sprite_smc_2:
	call $ffff		; SMC: patched to _sp1_draw_load1lb or _sp1_draw_mask2lb
				; no clean up - __z88dk_callee

	ld hl,(_pix_ptr)
jsp_sprite_smc_3:
	ld de,$ff		; SMC: patched to 8 (LOAD1) or 16 (MASK2)
	add hl,de
	...
	;         bg_ptr = &sp->pdbuf[ j * 8 ];
	;         for ( i = 0; i < sp->rows + 1; i++ ) {
	;             sp1_draw_load1( bg_ptr, pix_ptr, pix_ptr_left );
	;             bg_ptr += ( sp->cols + 1 ) * 8;
	;             pix_ptr += 8;
	;             pix_ptr_left += 8;
	;         }
...
jsp_sprite_smc_4:
	call $ffff		; SMC: patched to _sp1_draw_load1 or _sp1_draw_mask2
...
jsp_sprite_smc_5:
	...
	;         for ( i = 0; i < sp->rows + 1; i++ ) {
	;             sp1_draw_load1rb( bg_ptr, pix_ptr );
	;             bg_ptr += ( sp->cols + 1 ) * 8;
	;             pix_ptr += 8;
	;         }
	;     }
...
jsp_draw_sprite_right_i:
	...
	push hl			; param: pix_ptr
jsp_sprite_smc_6:
	call $ffff		; SMC: patched to _sp1_draw_load1rb or _sp1_draw_mask2rb
				; no cleanup - __z88dk_callee
