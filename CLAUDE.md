...
Tests are integration tests that run visually on the ZX Spectrum emulator — there is no automated test runner. To run a specific test:

1. Edit `main.c` and uncomment the desired test function call (only one at a time)
2. Run `make run` to build and launch FUSE

Available tests: `test_dtt`, `test_btt_contents`, `test_btt_redraw`, `test_sprite_draw`, `test_sprite_move`, `test_pool_and_colour`, `test_tiles_and_print`.
...
| FTT (Foreground Tiles Table) | 96 B | Bit per cell: 1 = foreground tile, never overwritten by sprites |
| BAT (Background Attributes Table) | 768 B | Color attributes per cell |
...
2. `jsp_redraw()`: walk DTT & FTT byte-by-byte; for each dirty non-foreground cell, draw from DRT to screen, reset DTT bit, restore DRT pointer from BTT
...
5D00-E59F  Program code/data (~34 KB free)
```

**Key implementation files**:
- `lib/jsp_redraw.asm` — Screen refresh loop (hot path, performance-critical)
