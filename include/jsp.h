...
//
// Size is given in PIXELS (width first, then height — i.e. W x H), and must be a
// whole number of 8x8 cells: both dimensions are asserted to be multiples of 8
// at compile time (a negative-sized typedef fires otherwise).  rows/cols are
// derived from the active mode, so one source builds unchanged on ZX and every
// CPC mode.
#define DEFINE_SPRITE(_name,_width_px,_height_px,_pixels,_xpos,_ypos,_type) \
