...
(where it was proven in Phase 4); see `doc/legacy/CPC-TARGET-PLAN.md` §11 for how it
...
(`doc/legacy/CPC-TARGET-PLAN.md` §1.2, §7, Phase 0). Build through the Makefile matrix:
...
- **Link org / memory map**: use the JSP-CPC designed memory map (the data block
  must sit **below the 0xC000 screen**, see `doc/legacy/CPC-TARGET-PLAN.md` §9) — set it
...
- **JSP writes its own CPC kernels** (per-mode shift tables + `sp1_draw_*`
  composite kernels, `doc/legacy/CPC-TARGET-PLAN.md` §4/§5). Unlike RAGE1 it does **not**
...
indices (`doc/legacy/CPC-TARGET-PLAN.md` §6) map to the intended inks. A screenshot is
...
- Build one disk per compilation mode (`CPC_MODE0/1/2`, `_MONO`, `_FAST`) via the
  ...
  (`doc/legacy/CPC-TARGET-PLAN.md` §11). Screenshot each and eyeball-verify.
...
- There is **no per-routine T-state profiler** for CPC equivalent to the JNEXT
  ...
  **wall-clock-timed** via the `-DTIME_LIMITED=N` + `tools/cap32-time.sh`
  ...
  (`doc/legacy/CPC-TARGET-PLAN.md` §11, risk 5).
