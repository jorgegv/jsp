...
# byte format documented in doc/CPC-ASSETS-FORMAT.md.
#
...
# per-mode geometry + grids (file-scoped: shared by the emit helpers below)
my ($ppc, $subcols, $nplanes);
my ($is_mask, $is_imask, $want_transp, $is_tile);
...
#####################
##
## Main entry point (called by the gfxgen.pl driver: --platform cpc)
##
#####################
...
sub run {
    ...
    Getopt::Long::Configure("default", "no_auto_abbrev", "bundling");
    GetOptions(
        'input|i=s'        => \$opt_input,
        'xpos|x:i'         => \$opt_xpos,
        'ypos|y:i'         => \$opt_ypos,
    ...
    defined($opt_input) && defined($opt_width) && defined($opt_height) &&
        ...
        or die "usage: gfxgen.pl --platform cpc -i PNG -x X -y Y --width W --height H -s SYM -g sprite_mask|sprite_load|tile [--mode 0|1] [--extra-bottom-row] [--extra-top-rows] [-m/-f/-b RRGGBB] [--multicolor [--palette-symbol SYM]]\n";
    ...
    $is_mask  = ($opt_gfx_type eq 'sprite_mask');
    ...
    $is_imask = ($opt_gfx_type eq 'sprite_imask');
    $want_transp = ($is_mask || $is_imask);   # extract a transparency grid?
    $is_tile  = ($opt_gfx_type eq 'tile');
    ...
    $ppc     = ($opt_mode == 1) ? 4 : 2;
    $subcols = 8 / $ppc;                  # cells per 8-px source column (2 or 4)
    $nplanes = 8 / $ppc;                  # planes per byte (2 for M1, 4 for M0)
    ...
    if ($opt_multicolor) {
        ...
        # ---- 2-colour path: extract via ZXGfx (shared with the ZX platform) ----
