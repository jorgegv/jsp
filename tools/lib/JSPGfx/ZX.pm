package JSPGfx::ZX;
#
# ZX Spectrum graphics emitter (1bpp) — the `--platform zx` path of gfxgen.pl.
#
# Body moved verbatim from the former standalone tools/gfxgen.pl; the only
...
gfxgen.pl --platform zx - Create ZX Spectrum graphics C/ASM data definitions from a PNG file
...
#####################
##
## Main entry point (called by the gfxgen.pl driver: --platform zx)
##
#####################

sub run {
    local @ARGV = @_;
    ## ZX path uses Getopt::Long defaults (auto_abbrev on, no bundling); reset in
    ## case the driver left a non-default configuration in place.
